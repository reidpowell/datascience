...
sql <- "SELECT year, SUM(Emissions) AS Total_Emissions FROM dat WHERE fips = 24510 GROUP BY year ORDER BY year ASC"
plot_dat <- sqldf(sql)
...
plot(plot_dat$year, plot_dat$Total_Emissions,
     ...
     main = "Total Emissions from PM2.5, Baltimore City MD, 1999--2008"
    )
