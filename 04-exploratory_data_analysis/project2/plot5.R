...
sql <- "SELECT year, SUM(Emissions) AS Total_Emissions FROM dat JOIN cod ON dat.SCC = cod.SCC WHERE [EI.Sector] LIKE '%-Road%' AND fips = 24510 GROUP BY year ORDER BY year ASC"
plot_dat <- sqldf(sql)
